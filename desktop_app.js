...
const TRANSLATIONS = {
  en: {
    ...
    navLogout: "Log Out",
    heroPill: "FIPS 140-3 Hybrid & NIST FIPS 203/204/205 Ready",
    heroTitlePrefix: "Fortify Against the",
    heroTitleHighlight: "Quantum Threat (Q-Day)",
    heroTitleSuffix: "Before It's Decrypted",
    heroDesc: "Global enterprise compliance & automated cryptographic migration lifecycle. FIPS 140-3 Hybrid & NIST FIPS 203/204/205 Ready. Discover legacy RSA/ECC vulnerabilities, model Store-Now-Decrypt-Later (SNDL) exposure, and deploy NIST-standard PQC hybrid ciphers.",
    ...
    metric2Val: "FIPS 140-3",
    metric2Label: "Hybrid & NIST 203/204/205 Ready",
    metric3Val: "< 120ms",
  ...
  id: {
    ...
    navRegister: "Daftar",
    navLogout: "Keluar",
    heroPill: "Siap FIPS 140-3 Hybrid & NIST FIPS 203/204/205",
    ...
    heroDesc: "Kepatuhan global dan siklus hidup migrasi kriptografi otomatis. Siap FIPS 140-3 Hybrid & NIST FIPS 203/204/205. Deteksi kerentanan RSA/ECC lawas, modelkan risiko Store-Now-Decrypt-Later (SNDL), dan terapkan cipher hibrida PQC standar NIST.",
    ...
    metric2Val: "FIPS 140-3",
    metric2Label: "Siap Hybrid & FIPS 203/204/205",
    metric3Val: "< 120ms",
...
function _renderDesktopHeaderBar(nodeId, licStatus) {
  ...
  if (userPill) {
    // Hide the "Tenant Admin" user menu pill completely on desktop
    userPill.style.display = 'none';
  }

  let bar = document.getElementById('desktop-sovereign-bar');
  if (!bar) {
    bar = document.createElement('div');
    bar.id = 'desktop-sovereign-bar';
    const navUserActions = document.getElementById('nav-user-actions');
    if (navUserActions) navUserActions.appendChild(bar);
  }

  const plan = (licStatus?.plan || 'COMMUNITY').toUpperCase();
  const isEnt = plan.includes('ENTERPRISE') || plan.includes('SOVEREIGN');
  ...
  const planLabel = isEnt ? '🛡️ ENTERPRISE SOVEREIGN' : '🌱 COMMUNITY (FREE)';

  bar.style.cssText = 'display:inline-flex;align-items:center;gap:0.55rem;font-size:0.75rem;';
  bar.innerHTML = `
    <span style="display:inline-flex;align-items:center;gap:0.35rem;padding:4px 10px;border-radius:6px;background:rgba(6,182,212,0.1);border:1px solid rgba(6,182,212,0.25);color:#06b6d4;font-family:var(--font-mono);font-weight:600;">
      <span style="width:6px;height:6px;border-radius:50%;background:#06b6d4;box-shadow:0 0 6px #06b6d4;"></span>
      Air-Gap: &lt;0.1ms
    </span>
    <span style="padding:4px 10px;border-radius:6px;background:${badgeBg};border:1px solid ${badgeBorder};color:${badgeColor};font-family:var(--font-mono);font-weight:700;">
      ${planLabel}
    </span>
    <span style="padding:4px 10px;border-radius:6px;background:rgba(255,255,255,0.04);border:1px solid var(--border-color);color:var(--text-muted);font-family:var(--font-mono);font-size:0.75rem;">
      Node: ${nodeId ? nodeId.split('-').slice(0, 3).join('-') : 'LOCAL'}
    </span>
  `;
}
...
async function loadTenantSubscription() {
  const isDesktop = location.hostname === '127.0.0.1' && (location.port === '9527' || location.port === '8080');
  if (isDesktop) {
    const lic = window._licenseStatus || { plan: 'COMMUNITY', status: 'ACTIVE' };
    const isEnt = (lic.plan || '').toUpperCase().includes('ENTERPRISE') || (lic.plan || '').toUpperCase().includes('SOVEREIGN');
    ...
    if (titleElem) titleElem.textContent = isEnt ? "Enterprise Sovereign Node" : "Community Edition (Free)";
    if (cycleElem) cycleElem.textContent = isEnt ? "Node-Locked • Air-Gapped Workstation" : "Local Sovereign Workstation • Unlimited";
    if (assetsElem) assetsElem.textContent = isEnt ? "Unlimited Local Assets" : "Local Workspace Scanner";
    if (agentsElem) agentsElem.textContent = isEnt ? "Multi-Core High-Throughput" : "Local Node Scanner";
    if (supportElem) supportElem.textContent = isEnt ? "Enterprise Priority Support" : "Community Open Source";
    if (statusElem) statusElem.textContent = "ACTIVE (Air-Gapped)";
    if (expiryElem) expiryElem.textContent = lic.expires_at ? `Valid until ${lic.expires_at}` : "Permanent / Non-Expiring";

    const btnViewPlans = document.querySelector("#tab-tenant-license button[onclick*='pricing']");
